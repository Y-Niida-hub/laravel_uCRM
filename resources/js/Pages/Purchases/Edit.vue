...
const props = defineProps({ 'order': Array, 'items': Array, errors: Object })
...
const itemList = ref([]) // 購入商品リスト


const form = reactive({
    id: props.order[0].id,
    date: dayjs(props.order[0].created_at).format("YYYY-MM-DD"),
    customer_id: props.order[0].customer_id, status: props.order[0].status, items: []
})
...
const updatePurchase = id => {
    itemList.value.forEach(item => {
        if (item.quantity > 0) // 数量が0より大きい場合のみ追加
            form.items.push({ id: item.id, quantity: item.quantity })
    })
    Inertia.put(route('purchases.update', { purchase: id }), form) // フォーム送信
}
                                        ...
                                        <div class="flex flex-wrap -m-2">
                                            <div class="p-2 w-full">
                                                <div class="relative">
                                                    ...
                                                    <input disabled id="date" type="date" name="date" :value="form.date"
                                            ...
                                            <div class="p-2 w-full">
                                                <div class="relative">
                                                    ...
                                                    <input disabled id="customer" type="text" name="customer"
                                            ...
                                            <div class="p-2 w-full">
                                                <div class="relative">
                                                    ...
                                                    <input id="status" type="radio" v-model="form.status" name="status"
                                                        value="1">未キャンセル
                                                    <input id="status" type="radio" v-model="form.status" name="status"
                                                        value="0">キャンセルする
                                                </div>
